...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const world = new CANNON.World({
        gravity: new CANNON.Vec3(0, -6.81, 0)
    });
...
logoLoader.load(
    ...
    (gltf) => {
        ...
        // Вместо добавления обработчика к модели, добавляем его к рендереру
        renderer.domElement.addEventListener('click', (event) => {
            // Проверяем, был ли клик по логотипу
    ...
    // Загрузка моделей с УВЕЛИЧЕННОЙ скоростью следования
    loadModelWithPhysics('threeD/bottle.glb', [0, 2, 3], [4, 4, 4], 0.05);
    loadModelWithPhysics('threeD/ice.glb', [3, 2, 3], [0.5, 0.5, 0.5], 0.08);
    loadModelWithPhysics('threeD/machine.glb', [-1, 2, 2.7], [1.1, 1.1, 1.1], 0.04);
    loadModelWithPhysics('threeD/tomato.glb', [0.1, 2, 2.8], [0.6, 0.6, 0.6], 0.07);
    loadModelWithPhysics('threeD/cow.glb', [1, 2, 3], [0.5, 0.5, 0.5], 0.09);
    ...
    function onMouseMove(event) {
        ...
        // Обновление целей для объектов с движением по всем осям
        followMouseObjects.forEach(obj => {
            ...
            obj.currentTarget.z = obj.originalPosition[2] + (mouseX * 1.5) - (mouseY * 1.0);
        });
    }
    ...
    function animate() {
        ...
        // Анимация следования объектов за мышью
        followMouseObjects.forEach(obj => {
            ...
            // Вращение с учетом движения по Z
            obj.body.angularVelocity.set(
                (mouseY * 0.03) + (Math.random() * 0.02 - 0.01),
                (mouseX * 0.03) + (Math.random() * 0.02 - 0.01),
                (mouseX + mouseY) * 0.02
            );
        });
